...
Երբևմն մենք նաև պետք է մուտք գործենք օրիգինալ DOM event inline հայտարարության handler-ում։ Դւոք կարող եք փոխանցել այն մեթոդին օգտագործելով հատուկ `$event` փոփոխականը․
